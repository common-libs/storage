...
class directory {
    ...
    /**
     * absolute path to the greatest dir this class should work on
     *
     * @var string
     * @since 1.1.0
     */
    ...
    public function __construct($dir = NULL, $cache = true)
    {
        ...
        $this->relativePath = str_replace("vendor" . DS . "profenter" . DS . "tools", "", $this->relativePath);
        $this->relativePath = common::fixPaths($this->relativePath);
        if (!$this->exists()) {
            throw new FileNotFoundException(common::fixPaths($this->getBasePath() . $this->getDir()));
        }
        return $this;
    }

    /**
     * scans the given dir to array, uses cache functions
     */
        ...
        try {
            ...
            if (is_null($files) or !$cache) {
                ...
                foreach ($aFiles as $file) {
                    $files[str_replace(common::fixPaths($this->getBasePath() . $this->getDir()), "", $file->getPathname())] = str_replace($this->getBasePath(), "", $file->getPathname());
                }
            ...
            echo $e->getMessage();
            echo "<br>Path: " . $this->getDir() . DS . ".profenter" . DS . "simpledirlister" . DS . "cache";
     ...
     * @uses  tools
     */
    public function infos($recursion = false, $array = [])
    {
        $a = [];
        if (empty($array)) {
            $array = $this->files;
        ...
        foreach ($array as $index => $value) {
            ...
            if (is_dir($this->getBasePath() . "/" . $v)) {
                ...
                $a[$index] = [
                    "time" => "---",
                    "path" => common::fixPaths($this->getBasePath() . DS . $path),
                ...
                $url = implode(DS, $e);
                $a[$index] = [
                    "path" => common::fixPaths($this->getBasePath() . DS . $value),
                    "url" => url::parse([], $base . parse_url($url, PHP_URL_PATH) . $value),
                    "time" => date('Y-m-d H:i:s', filemtime($this->getBasePath() . DS . $value)),
                    "type" => "file",
                    "extension" => $file->getExtension(),
                    "size" => $file->getSize(),
                    "json" => json_encode([
                        "size" => $file->getSize(),
                        "md5" => md5_file($this->getBasePath() . DS . $value),
                        "sha1" => sha1_file($this->getBasePath() . DS . $value),
                    ])
                ];
            }
        }
    ...
    public function isHidden($path)
    {
        ...
        foreach ($this->ignore as $hiddenPath) {
            //echo $hiddenPath . "----".$path . "-----".var_dump(fnmatch($hiddenPath, $path)) . "\n";
        ...
        foreach ($e as $item) {
            if (!empty($item)) {
                if ($item == "..") {
                    ...
                    $this->setDir(common::fixPaths($this->getDir() . DS . $item));
                } else {
