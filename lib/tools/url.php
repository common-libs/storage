 ...
 * @since   1.0 based on
 *          https://github.com/profenter/simpleDirLister/blob/9e5782fed9631575c2ce5122c120e8db7c5a13ee/include/classes/url.php
 */
class url
{
	/**
	...
	public static function parse($array = [], $host = NULL)
	{
		...
		if (is_null($host)) {
			...
			$url = parse_url($host, PHP_URL_SCHEME) . '://' . parse_url($host, PHP_URL_HOST) . (parse_url($host, PHP_URL_PORT) != "80" ? ":" . parse_url($host, PHP_URL_PORT) : "") . parse_url($host, PHP_URL_PATH) . "?" . http_build_query($query, '', '&');
		}
		$url = rtrim($url, "?");
